...
本次迭代对 daily-workflow 进行了存储架构和流程交互的全面重构，核心方向是"去中心化 + 本地优先 + 精简机制"。

### 本次优化清单

| 序号 | 优化项 | 说明 |
|------|--------|------|
| 1 | 文档输出目录统一 | 所有产出物从 `.kiro/skills/daily-workflow/output/` 迁移到项目根目录 `dw-docs/`，skill 目录只放定义和规范 |
| 2 | 去除飞书绑定 | 飞书从默认输出后端降级为可选同步渠道，仅在设计文档（第二步）和提测（第七步）完成后询问用户是否需要同步 |
| 3 | 待确认项交互 | 需求分析和设计文档阶段新增"选择题"机制——有矛盾/歧义时直接在 md 文档末尾写出选项，用户在文件中作答后 AI 继续 |
| 4 | 需求文件夹归拢 | 每个需求的所有文档统一放在 `dw-docs/{需求名}/` 子文件夹，中文命名，移交时整个文件夹交出即可 |
| 5 | Done 区季度归档 | dw-daily.md 的 Done 区只保留本季度已完成需求，季度结束时归档到 `dw-docs/archive/YYYY-QX.md` |
| 6 | 轻量维护策略 | 砍掉"索引刷新"和"文件夹完整性检查"（过度维护），只保留日报时归档检查 + 周报时过期提醒（2周无更新） |
| 7 | 自我学习精简 | 删除 feedback-loop.md 和 LEARNING.md 中的复杂反馈日志机制，改为"对话中识别 → 问用户 → 改规范 → 记一行"四步闭环 |
| 8 | output 目录清除 | 删除 `.kiro/skills/daily-workflow/output/` 整个目录，已有 dw-state.json 和 dw-daily.md 迁移到 `dw-docs/` |
| 9 | dw-state.json 清理 | 删除已废弃的 `storage_backend: "feishu"` 字段 |
| 10 | 度量埋点降级 | 从"遗漏视为执行失败"降为"建议记录，不强制"，减少流程负担 |
| 11 | 删除飞书机器人通知 | 删除 `bot-notification.md`，飞书已降级为可选，通知机制无意义 |
| 12 | 跟进模式建文件夹 | 跟进模式也创建 `dw-docs/{需求名}/` 文件夹，方便回顾和移交 |

### 变更动机

- **output 目录在 skill 内部不合理**：产出物应该跟项目走，不应该藏在 .kiro 里面
- **飞书强绑定是痛点**：Token 过期、MCP 不稳定、文档精确编辑受限，且大部分文档不需要共享
- **需求移交应该极简**：文件夹拷贝 > 飞书权限管理
- **feedback-loop 形式大于实质**：日志空空，AI 实际不会每次都去"读历史反馈"，真正有效的学习是直接改规范
- **gardening 不应该过度设计**：索引在流程中已同步维护，文件夹按阶段产出本就不完整

### 变更前后对比

| 维度 | 变更前 | 变更后 |
|------|--------|--------|
| 文档输出位置 | `.kiro/skills/daily-workflow/output/` | `{PROJECT_ROOT}/dw-docs/` |
| 飞书文档 | 默认输出（需配置 MCP） | 可选同步（仅设计+提测阶段询问） |
| 需求文件组织 | `dw-docs/[需求名]_需求分析.md` 平铺 | `dw-docs/{需求名}/requirements.md` 文件夹 |
| 需求移交 | 生成移交文档 + 飞书通知 | 整个文件夹交给接手人 |
| 自我学习 | feedback-loop.md + LEARNING.md 日志表 + 策略优化表 | 对话中识别 → 问 → 改规范 → LEARNING.md 记一行 |
| Done 区膨胀 | 无限累积 | 按季度归档到 archive/ |
| 维护机制 | 索引刷新 + 文件夹检查 + 状态归档 + 过期清理 | 日报时归档 + 周报时过期提醒 |
...
```
对话中识别信号 → 问用户"要不要沉淀为规范？" → 改对应文件 → LEARNING.md 记一行
```

| 识别信号 | 示例 |
|----------|------|
| 用户纠正了 AI 的做法 | "不要用 moment，用 dayjs" |
| 用户补充了遗漏内容 | "这里还要加 loading 状态" |
| 同一问题第二次出现 | 又忘了加空值保护 |
| 用户表达偏好 | "组件命名用大驼峰" |
| 用户跳过了步骤 | "跳过单测，列表页不需要" |
| 发现工具限制 workaround | "文件是 CRLF，脚本要按行处理" |

不记 feedback log，不搞统计分析，不定期回顾。

### 轻量维护策略

| 时机 | 维护动作 |
|------|----------|
| 输出日报时 | 归档检查：已上线需求 Doing→Done，状态不一致以 dw-state.json 为准修复 |
| 写周报时 | 过期提醒：列出 Doing 区超过 2 周无更新的需求 |
| 季度总结时 | 季度归档：Done 区内容移到 `archive/YYYY-QX.md`，Done 区清空 |

不做索引一致性扫描（流程中已同步维护），不做文件夹完整性检查（按阶段产出本就不完整）。

> **小结**：本次迭代的核心理念是"做减法"——去掉飞书强绑定、去掉复杂的 feedback 机制、去掉过度的 gardening、去掉产出物和定义的耦合。同时做了几项清理：dw-state.json 去除废弃字段、度量从强制降为建议、删除无用的机器人通知、跟进模式也建文件夹。最终形态：文档跟项目走（dw-docs/）、飞书按需同步、学习靠直接改规范、移交靠文件夹拷贝、维护挂在自然时机上。更简单，更实用。
